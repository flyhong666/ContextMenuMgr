...
`ContextMenuMgr.ProbeHost` 是短生命周期隔离进程。它可以崩溃或超时，而不影响前端、后端服务和普通菜单管理。
...
| `ProbeHostDependencyMissing` | framework-dependent ProbeHost 缺少 `ContextMenuMgr.Contracts.dll` 等运行依赖。 |
...
| `ProbeHostManagedUnhandledException` | ProbeHost 内部托管未处理异常。 |
| `Timeout` | 超过默认或传入超时时间，前端会尝试结束进程。 |
| `InvalidProbeHostJson` / `InvalidProbeHostOutput` | ProbeHost 未返回合法 JSON。 |

## 8. 构建与部署注意事项

ProbeHost 必须随前端部署多架构目录。当前 `ContextMenuMgr.Frontend.csproj` 在 framework-dependent Debug/Build 场景会构建并复制 `ProbeHost\x86`、`ProbeHost\x64`、`ProbeHost\arm64`，并检查每个目录至少包含：

- `ContextMenuMgr.ProbeHost.exe`
- `ContextMenuMgr.ProbeHost.dll`
- `ContextMenuMgr.ProbeHost.deps.json`
- `ContextMenuMgr.ProbeHost.runtimeconfig.json`
- `ContextMenuMgr.Contracts.dll`

Release 发布由 `Scripts/Build.Common.psm1` 的 `Get-ProbeHostArchitectureMap` 决定携带哪些架构。`win-x86` 只带 x86，`win-x64` 带 x64 和 x86，`win-arm64` 带 arm64、x64 和 x86。当前脚本对 ProbeHost 使用 `--self-contained true` 和 `-p:PublishSingleFile=false`，因此 Release 产物是 self-contained 多文件输出。
