...
| --- | --- | --- | --- | --- | --- | --- |
| 链路 A | Frontend -> Backend Pipe -> Service | 快照、传统菜单开关、审核、SpecialMenu、Win11 blocked list、AutoStart 运行时读写、Restart Explorer | 前端不提权，服务执行高权限部分 | 用户级操作需要 | 仅 Session 相关操作需要 | `NamedPipeBackendClient.cs`、`NamedPipeBackendServer.cs`、`BackendUserContextResolver.cs` |
| 链路 B | Frontend -> UAC Bootstrapper | 安装/修复服务、卸载服务、停止服务、设置服务启动模式 | 是，通过 `Verb=runas` | `install-or-repair` 和 `set-startup-mode` 需要 `--user-sid` | 通常不需要 | `BackendServiceManager.cs`、`BackendServiceBootstrapper.cs` |
...
| 链路 D | Frontend -> ProbeHost | Deep Analysis，隔离第三方 Shell Extension COM 风险 | 否 | 不需要 | 不需要 | `ContextMenuDeepAnalysisService.cs`、`ContextMenuMgr.ProbeHost/Program.cs` |
...
不要用这条链路做普通菜单开关、Win11 禁用、SpecialMenu 修改、AutoStart 运行时读写或 Restart Explorer。它的职责是服务生命周期维护，不是 runtime backend。
...
架构选择由前端完成。`ContextMenuDeepAnalysisService` 会读取目标 handler DLL 的 PE machine type，选择 `ProbeHost\x86`、`ProbeHost\x64` 或 `ProbeHost\arm64` 下的 `ContextMenuMgr.ProbeHost.exe`。如果 handler 架构未知，当前实现倾向于回退到当前前端进程架构。启动前还会验证 ProbeHost exe 架构和依赖文件，例如 `ContextMenuMgr.ProbeHost.runtimeconfig.json`、`ContextMenuMgr.ProbeHost.deps.json`、`ContextMenuMgr.Contracts.dll`。
...
| Win11 新菜单禁用/恢复 | 链路 A | 是 | 否 | user blocked list 必须带 `BackendUserContext`，机器级另有 HKLM blocked list。 |
