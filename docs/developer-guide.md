...
| ProbeHost | `ContextMenuMgr.ProbeHost/Program.cs` | COM 探测、SpecificHandler/WholeContextMenu、菜单枚举 | 不要把它改成提权进程或注册表写入工具。 |
...
ProbeHost 支持 `x86`、`x64`、`arm64`。选择依据主要是 handler DLL 的 PE machine type；unknown 时当前实现倾向于当前前端进程架构。`ContextMenuMgr.Frontend.csproj` 和 `Scripts/Build.Common.psm1` 都有多架构 ProbeHost 发布逻辑。
...
| ProbeHost 发布 | 按平台发布多架构。`win-x64` 包含 x64 + x86；`win-arm64` 包含 arm64 + x64 + x86；framework-dependent portable 的 anycpu 路径由前端 csproj 构建 x86/x64/arm64。Release 脚本对 ProbeHost 使用 `PublishSingleFile=false`，因此是 self-contained 多文件输出，不是 single-file。 |
...
`ContextMenuMgr.Frontend.csproj` 也定义了 framework-dependent 构建时的 ProbeHost 多架构目标：构建后复制到 `ProbeHost\x86`、`ProbeHost\x64`、`ProbeHost\arm64`，并验证 `.dll`、`.deps.json`、`.runtimeconfig.json` 和 contracts 依赖。
...
| ProbeHost 缺依赖 | `ContextMenuDeepAnalysisService.cs`、`ContextMenuMgr.Frontend.csproj` | `frontend-debug.log` | 发布布局缺 `.deps.json`、`.runtimeconfig.json` 或 contracts。 |
...
| 深入分析失败 | `ContextMenuDeepAnalysisService.cs`、`ContextMenuMgr.ProbeHost/Program.cs` | `frontend-debug.log` 的 ProbeHost diagnostics | 把 COM 探测失败当成菜单管理失败。 |
