...
SCM 启动 ContextMenuManagerPlus.Service.exe --service
  ...
  -> BackendRuntime.StartAsync
      ...
      -> RuntimeDataAclRepairService 修复 %ProgramData%\ContextMenuMgr ACL
      -> FileLogger
...
`RuntimePaths` 当前把运行时根目录定义为 `%ProgramData%\ContextMenuMgr`。这个目录是前端、后端服务和 TrayHost 共享的运行时数据目录，普通用户被有意授予 Modify 权限以支持 portable 部署和既有安装自修复。`FrontendSettingsService` 读写 `%ProgramData%\ContextMenuMgr\frontend-settings.json`，并会从旧的 `%LOCALAPPDATA%\ContextMenuMgr\frontend-settings.json` 迁移。日志统一在 `%ProgramData%\ContextMenuMgr\Logs` 下，状态库是 `%ProgramData%\ContextMenuMgr\context-menu-state.json`，删除备份目录是 `%ProgramData%\ContextMenuMgr\DeletedBackups`。旧版本可能使用 legacy 路径，当前代码保留迁移/兼容路径常量。

后端启动时会在创建 `FileLogger`、`ContextMenuStateStore` 和 `BackendProtectionSettingsStore` 前 best-effort 调用 `RuntimeDataAclRepairService`。前端保存设置遇到 ACL 相关的 `UnauthorizedAccessException` / `IOException` / `SecurityException` 时，会先通过 `PipeCommand.RepairRuntimeDataAcl` 请求后端修复并重试一次；如果 pipe 不可用，再通过 UAC bootstrapper 的 `repair-runtime-data-acl` 命令做独立 fallback。该修复只作用于 `%ProgramData%\ContextMenuMgr` 文件系统 ACL，不是 Registry Write Protection，也不是 ShellNew ACL Lock。
